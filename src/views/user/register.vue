<template>
  <div class="page">
    ...
    <div class="input-wrap">
      ...
      <div class="line"></div>
      <input v-model="formData.phone" type="text" placeholder="请输入手机号" class="input" style="flex: 1; margin-right: 1.28rem" />
    </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      bindAccount({ openid: this.OPEN_ID, phone: this.formData.phone, code: this.formData.code }).then(res => {
        if (res.code !== 1) {
          this.$toast(res.msg);
        } else {
